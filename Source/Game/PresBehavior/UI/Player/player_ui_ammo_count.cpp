    ...
    const PlayerUiSettings::AmmoCountSettings& settings)
{
    ...
    if (auto* image = m_fill.handle.GetImage()) image->SetColor({0.9f, 0.9f, 0.2f, 1});
...
{
    if (!m_group) return;
    if (auto* image = m_fill.handle.GetImage()) image->SetFillAmount(static_cast<float>(m_ammoCount) / 20 * 0.75f);
    if (auto* text = m_currentText.handle.GetText()) text->SetText(std::to_string(m_ammoCount));
    UpdateMarkers();
}
