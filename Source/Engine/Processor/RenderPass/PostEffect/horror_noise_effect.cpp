#include "horror_noise_effect.h"

#include <algorithm>
#include "Engine/engine_service_locator.h"

void HorrorNoiseEffect::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
    m_context = context;
    m_constantBuffer = SHADER_REPOSITORY->GenerateConstantBufferResource(
        "HorrorNoiseBuffer", 0, sizeof(ConstantBufferData), false, true, ConstantBufferUsage::Dynamic);

    ShaderProgramResource resource;
    resource.name = "HorrorNoiseEffect";
    resource.baseShader = SHADER_REPOSITORY->GetShaderProgramResource(ShaderBase::FullScreen);
    resource.overridePixelShader = SHADER_REPOSITORY->GetPixelShaderResource("horror_noise_ps.cso");
...
void HorrorNoiseEffect::Process(ID3D11ShaderResourceView* inputSRV, ID3D11RenderTargetView* outputRTV,
    float noiseMin, float noiseMax, float contrastPow, float strength, float time)
{
    ...
    data.noiseMin = noiseMin;
    data.noiseMax = noiseMax;
    data.contrastPow = (std::max)(contrastPow, 0.001f);
